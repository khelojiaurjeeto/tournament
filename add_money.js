const username = localStorage.getItem('khelo_local');
if (!username) {
    alert('Please set username in local storage first!');
}

const SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbxgbOKrwHJnLyCxp0F3gggrFmkkq3_QKjb9M-OnL5acZydgX5Nu-uWg_OkeDvJW2iB-zw/exec';
...
function showQR() {
    ...
    const upiId = '6362359577@naviaxis';
...
async function uploadToImgur(file) {
    ...
    const response = await fetch('https://api.imgur.com/3/image', {
        method: 'POST',
        headers: {
            'Authorization': 'Client-ID 5b72c60d5caf614'
        },
        body: formData
    });
